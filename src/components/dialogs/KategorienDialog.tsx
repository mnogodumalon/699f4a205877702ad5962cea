...
export function KategorienDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = false }: KategorienDialogProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      await onSubmit(fields as Kategorien['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  return (
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{defaultValues ? 'Kategorien bearbeiten' : 'Kategorien hinzufügen'}</DialogTitle>
        </DialogHeader>
